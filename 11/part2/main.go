...
func countNeighbors(grid map[[2]int]string, x int, y int) int {
	xMax := 0
	yMax := 0
	for coords, _ := range grid {
		if coords[0] > xMax {
	...
	for dx := -1; dx <= 1; dx++ {
		for dy := -1; dy <= 1; dy++ {
			...
			for c := 1; c < 100; c++ {
				...
				switch state := grid[[2]int{x + c*dx, y + c*dy}]; state {
				case "#":
					neighborCount++
					done = true
				case "L":
				...
				case ".":
					// need to look further in this direction
					continue
				}
			}
...
func step(grid map[[2]int]string) map[[2]int]string {
	...
	for x := 0; x <= xMax; x++ {
		for y := 0; y <= yMax; y++ {
			...
			neighborCount := countNeighbors(grid, x, y)
			if isEmpty && neighborCount == 0 {
				nextGrid[[2]int{x, y}] = "#"
			} else if !isEmpty && neighborCount >= 5 {
